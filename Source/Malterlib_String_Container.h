			...
			void fp_AddFormat(TICStrFormatType<TCFormat> *_pFormat, mint _Flags)
			{
				...
				if (m_nFormats >= m_lFormats.f_GetLen())
				{
					if (!m_lFormats.f_GetLen())
					{
						...
						NMem::fg_MemCopy(m_lFormats.f_GetArray(), m_plFormats, m_nFormats * sizeof(void *));
					}
					else
						m_lFormats.f_SetLen(m_nFormats*2);
				}
...
EndArgSearch:
		...
		private:
			...
			typedef typename TCChooseType
				<
					NTraits::TCIsSame<typename CStrTraits::CAllocator, NMem::CAllocator_NonTrackedHeap>::mc_Value
					, CStrNonTracked
					, CStr
				>::CType CTempStr
			;
		...
		public:
			...
			typename TCEnableIf<t_CStrTraitsF::CStrTraits::mc_Type != EStrType_Ansi, void>::CType f_AddStr(const TCStrAggregate<t_CStrTraitsF> &_From)
			{
				if (*_From.f_GetStr() != 0)
				{
					aint Length = f_GetLen();
			...
			inline_extralarge void fp_AddStrLengthAware(const CAddStrAgrs &_Args, CChar const *_pStr)
			{
				...
				{
					NMib::NMem::fg_MemCopy((pWritable + _Args.m_StrLen), _pStr, (MaxLen - _Args.m_StrLen) * sizeof(CChar));
					_Args.m_StrLen = MaxLen;
					pWritable[MaxLen] = 0;
